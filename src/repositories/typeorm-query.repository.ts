  ...
  implements QueryRepository<Entity, DeepPartial<Entity>, DeepPartial<Entity>>
{
  ...
  constructor(readonly repo: Repository<Entity>, opts?: TypeOrmQueryRepositoryOpts<Entity>) {
    super();
  ...
  queryOne(context: IContext, filter: Filter<Entity>): Promise<Entity | undefined> {
    return this.filterQueryBuilder.select({ filter }).getOne();
  }

  async aggregate(context: IContext, filter: Filter<Entity>, aggregate: AggregateQuery<Entity>): Promise<AggregateResponse<Entity>[]> {
   ...
   */
  async findById(context: IContext, id: string | number, opts?: FindByIdOptions<Entity>): Promise<Entity | undefined> {
   ...
   * @param id - The id of the record to find.
   */
  async getById(context: IContext, id: string | number, opts?: GetByIdOptions<Entity>): Promise<Entity> {
   ...
   */
  async createOne(context: IContext, record: DeepPartial<Entity>): Promise<Entity> {
   ...
   */
  async createMany(context: IContext, records: DeepPartial<Entity>[]): Promise<Entity[]> {
   ...
   */
  async updateOne(context: IContext, id: number | string, update: DeepPartial<Entity>, opts?: UpdateOneOptions<Entity>): Promise<Entity> {
    this.ensureIdIsNotPresent(update);
    const entity = await this.getById(context, id, opts);
    return this.repo.save(this.repo.merge(entity, update as TypeOrmDeepPartial<Entity>) as TypeOrmDeepPartial<Entity>);
  }
   ...
   */
  async updateMany(context: IContext, update: DeepPartial<Entity>, filter: Filter<Entity>): Promise<UpdateManyResponse> {
    this.ensureIdIsNotPresent(update);
   ...
   */
  async deleteOne(context: IContext, id: string | number, opts?: DeleteOneOptions<Entity>): Promise<Entity> {
    const entity = await this.getById(context, id, opts);
   ...
   */
  async deleteMany(context: IContext, filter: Filter<Entity>): Promise<DeleteManyResponse> {
   ...
   */
  async restoreOne(context: IContext, id: string | number, opts?: Filterable<Entity>): Promise<Entity> {
   ...
   * @param filter - A `Filter` to find records to delete.
   */
  async restoreMany(context: IContext, filter: Filter<Entity>): Promise<UpdateManyResponse> {
    this.ensureSoftDeleteEnabled();
  ...
  private async ensureIsEntityAndDoesNotExist(e: DeepPartial<Entity>): Promise<Entity> {
    if (!(e instanceof this.EntityClass)) {
      return this.ensureEntityDoesNotExist(this.repo.create(e as TypeOrmDeepPartial<Entity>) as Entity);
    }
